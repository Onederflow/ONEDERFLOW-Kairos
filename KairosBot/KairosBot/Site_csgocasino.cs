...
namespace KairosBot
{
    public class Site_csgocasino
    {
        ...
        {
            HttpWebRequest rr = (HttpWebRequest)WebRequest.Create("http://csgocasino.net/api/site-inventory");
        ...
        {
            console.WriteLine(">>> csgocasino start loading");     //console
            ...
            console.WriteLine(">>> csgocasino data downloaded");     //console
            JArray jar = JObject.Parse(line).Value<JArray>("inventory");
            ...
            jar = JObject.Parse(line).Value<JArray>("prices");
            jtok = jar.First;

            while (jtok != null)
            {
                string name = jtok.Value<string>("market_hash_name");
                float price = (float)jtok.Value<int>("price") / 1000;

                for (int j = 0; j < res.count; j++)
                    if (res.item[j].name == name)
                    {
                        res.item[j].priceFrom = price;
                        res.item[j].priceTo = price;
                    };
                jtok = jtok.Next;
            };
            console.WriteLine(">>> csgocasino: ok");     //console
            return res;
        }
