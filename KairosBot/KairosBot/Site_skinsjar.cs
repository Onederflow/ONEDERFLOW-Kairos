...
namespace KairosBot
{
    public class Site_skinsjar
    {
        ...
        {
            HttpWebRequest rr = (HttpWebRequest)WebRequest.Create("https://skinsjar.com/api/v3/load/bots?refresh=1&v=2");
        ...
        {
            console.WriteLine(">>> skinsjar start loading");     //console
            ...
            console.WriteLine(">>> skinsjar data downloaded");     //console
            JArray jar = JObject.Parse(line).Value<JArray>("items");
            ...
            while (jtok != null)
            {
                ...
                {
                    string name = jtok.Value<string>("name");
                    string bot = jtok.Value<string>("bot");
                    string category = jtok.Value<string>("category");

                    float priceTo = 0;
                    switch (category)
                    {
                        case "Knife": priceTo = (float)Math.Round(price*0.95f,3);
                            break;
                        case "Sticker": priceTo = (float)Math.Round(price * 0.85f / 0.9f, 3);
                            break;
                        case "Rifle": priceTo = (float)Math.Round(price * 0.9f / 0.95f, 3);
                            break;
                        case "SMG": priceTo = (float)Math.Round(price * 0.9f / 0.95f, 3);
                            break;
                        case "Pistol": priceTo = (float)Math.Round(price * 0.9f / 0.95f, 3);
                            break;
                        case "Sniper Rifle":priceTo = (float)Math.Round(price * 0.9f / 0.95f, 3);
                            break;
                        case "Music Kit":priceTo = (float)Math.Round(price * 0.85f / 0.90f, 3);
                            break;
                        case "Shotgun": priceTo = (float)Math.Round(price * 0.9f / 0.95f, 3);
                            break;
                        case "Key": priceTo = (float)Math.Round(price * 0.99f / 1.04f, 3);
                            break;
                        case "Container":priceTo = (float)Math.Round(price * 0.85f / 0.9f, 3);
                            break;
                        case "Gloves": priceTo = (float)Math.Round(price * 0.85f / 0.90f, 3);
                            break;
                    }
                    jid = jtok.Value<JArray>("items").First;
                    while (jid != null)
            ...
            res.count = i;
            console.WriteLine(">>> skinsjar: ok");     //console
            return res;
        }
